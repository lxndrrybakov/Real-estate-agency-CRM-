...
import { Users, UserCheck, XCircle, CheckCircle, User, LogOut, Plus, Calendar as CalendarIcon, BarChart as ChartBar } from 'lucide-react';
...
const EMPLOYEES = [
  { id: '00000000-0000-0000-0000-000000000002', full_name: 'Наталья' },
  { id: '00000000-0000-0000-0000-000000000003', full_name: 'Роман' },
  { id: '00000000-0000-0000-0000-000000000004', full_name: 'Андрей' },
  { id: '00000000-0000-0000-0000-000000000005', full_name: 'Александр' },
];
...
function OwnerDashboard() {
  ...
  const filteredClients = clients.filter(client => {
    const matchesEmployee = selectedEmployee ? client.employee_id === selectedEmployee : true;
    const matchesStatus = client.status === activeTab;
    const matchesName = client.full_name.toLowerCase().includes(nameFilter.toLowerCase());
    const matchesPhone = client.phone?.toLowerCase().includes(phoneFilter.toLowerCase()) ?? true;
    const matchesInfo = (client.initial_info?.toLowerCase().includes(infoFilter.toLowerCase()) ?? true) ||
                       (client.progress_notes?.toLowerCase().includes(infoFilter.toLowerCase()) ?? true);

    return matchesEmployee && matchesStatus && matchesName && matchesPhone && matchesInfo;
  });

  const handleAddClient = async (clientData: Partial<Client>) => {
    if (!clientData.employee_id) return;
  ...
  const handleEditClient = async (updatedClient: Client) => {
    const updateData = {
      employee_id: updatedClient.employee_id,
      full_name: updatedClient.full_name,
      birth_date: updatedClient.birth_date || null,
      phone: updatedClient.phone,
      source: updatedClient.source,
      initial_info: updatedClient.initial_info,
      progress_notes: updatedClient.progress_notes,
      next_contact: updatedClient.next_contact || null,
    ...
    setClients(prev => prev.map(client => 
      client.id === updatedClient.id ? { ...client, ...updateData } : client
    ));
    setEditingClient(null);
    loadClients(); // Перезагружаем клиентов для синхронизации
  };
  ...
  const handleAddEvent = async (event: CalendarEvent) => {
    ...
    loadAllEvents(); // Перезагружаем события для синхронизации
  };
                    ...
                    <tr key={client.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {EMPLOYEES.find(emp => emp.id === client.employee_id)?.full_name}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <>
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  if (reason) {
                                    handleUpdateClientStatus(client, 'cancelled', reason);
                                  }
                                }}
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Планы сотрудников</h2>
              <button
